# Rob Hubbard's music routine — disassembled, commented and explained

**Source:** *C=Hacking* issue 5, article by Anthony McSweeney
(`u882859@postoffice.utas.edu.au`), transcribed verbatim from
`chacking5.html`. The commented 6502 listing is Rob Hubbard's own
*Monty on the Run* driver, which the article notes was used "with small mods"
for his first ~30 tunes.

> The editor's note in the original reads: *"I questioned this article
> concerning copyright problems and he has assured me that it is legal to
> present it in entirety like this as it is past a certain # of years."*

## Why this file is in this repository

H2G reverse-engineers this exact routine family from compiled bytes. This is
the author's own description of the same structures, and it independently
corroborates several readings this project arrived at from the 6502 alone —
which makes it a useful cross-check when a signature or a field offset is in
doubt.

The correspondences worth knowing, all stated by the article below:

| the article says | H2G's name for it |
|---|---|
| `mstatus`: `$40` init, `$80`/`$C0` off, else play | the status byte the `BIT`/`BMI`/`BVC` chain tests |
| track ends in `$FF` (loop) or `$FE` (play once) | `tracks.py`'s version 0/1/3 markers, and `--legal-restart` |
| note byte: length in the low 5 bits | the duration `W` of § 7.ii's drum sweep |
| note bit 5 = no release, bit 6 = append to last, bit 7 = instrument or portamento follows | the status-byte grammar, incl. `--status-bit6` |
| second byte positive = instrument, negative = portamento (bit 0 = direction) | the slide operand and `--slides` |
| instrument is an **8-byte** record | `det.instr_stride == 8` |
| byte 0/1 pulse width lo/hi, 2 control, 3 attack/decay, 4 sustain/release | the ADSR copied verbatim to the `.sng` |
| **byte 5 = vibrato depth** | `det.vibrato_offset`, found at `+5` in every corpus file |
| byte 6 = pulse speed | `--pulse`, `_pulse_program` |
| **byte 7 = instrument fx byte**, bit 0 drum, bit 1 "skydive", bit 2 **octave arpeggio** | `effect_drum` / `effect_rise` / `effect_arp` |

Two of those are worth calling out because this project reached them the hard
way. The article says byte 7 bit 2 is an **octave** arpeggio — "a very limited
arpeggio routine" — which is the fixed `ADC #$0C` dialect, distinct from the
nibble-derived interval Warhawk uses. And it confirms the drum as "a noise
channel and also a fast frequency down, with fast decay… only the first 50th
of a second is a noise channel", i.e. the leading noise tick and the downward
sweep, both of which had to be re-derived from traces.

It also names the routine's own limits plainly: "All the other bits have no
meaning in this music, but were used a lot in later music for the fx." So this
is the *early* dialect, and the later players this repo also converts diverge
from it — which is exactly why H2G gates each effect on finding the routine
that reads it rather than trusting the bit.
...
Rob Hubbard's *Monty on the Run* driver, as published in the article.
