...
`CMP #imm / BCS / JMP` at the entry) names {sfx_count} subtune(s) across
{sfx_files} file(s) as sound effects rather than music, which is a positive,
player-derived reason and not a garbage read past the table's end. That is
{sfx_pct}% of the {loss_total} lost below; the remaining {rest_pct}% is
...
def subtune_census(paths, log=lambda *a: None) -> list[dict]:
    """One record per declared subtune of every file, with its fate."""
    ...
    for path in paths:
        try:
            ...
            rows.append({"file": path.name, "subtune": 0, "fate": "unreadable",
                         "why": type(exc).__name__, "voices_ok": 0,
                         "refs": 0, "dangling": 0, "declared": 0})
            continue
        cen: list[dict] = []
        try:
            ...
            rows.append({"file": path.name, "subtune": 0, "fate": "unreadable",
                         "why": type(exc).__name__, "voices_ok": 0,
                         "refs": 0, "dangling": 0,
                         "declared": sid.subtunes})
            continue
        for r in cen:
            rows.append(dict(r, file=path.name, declared=sid.subtunes))
    return rows


def build_subtune_census(rows: list[dict], sid_dir, version: str) -> str:
    from collections import Counter, defaultdict

    lost = [r for r in rows if r["fate"] in ("trimmed", "placeholder")]
    beyond = [r for r in rows if r["fate"] == "beyond_table"]
    emitted = [r for r in rows if r["fate"] == "emitted"]
    files = sorted({r["file"] for r in rows})

    # For the headline paragraph: of everything the header declared but the
    # converter did not emit, how much is the player's init dispatch naming a
    # real, non-garbage reason (`sfx`) rather than a read past the track
    # table's end (every other non-emitted fate)? Computed from the same rows
    # the by-cause table below is built from, so the two cannot disagree, and
    # the sentence stays true if the corpus or the bounds change.
    loss_total = len(rows) - len(emitted)
    sfx_rows = [r for r in rows if r["fate"] == "sfx"]
    sfx_count = len(sfx_rows)
    sfx_files = len({r["file"] for r in sfx_rows})
    sfx_pct = round(100 * sfx_count / loss_total) if loss_total else 0
    rest_pct = 100 - sfx_pct if loss_total else 0

    out = [f"# Subtune census", "",
           f"Generated by `python/survey.py --subtune-census` (h2g {version}), "
           f"{len(files)} file(s) under `{sid_dir}`.", "",
           SUBTUNE_CENSUS_DOC.format(sfx_count=sfx_count, sfx_files=sfx_files,
                                      sfx_pct=sfx_pct, loss_total=loss_total,
                                      rest_pct=rest_pct), "",
           "## Totals", "",
           f"- declared by the PSID headers: **{len(rows)}**",
           f"- emitted: **{len(emitted)}**",
           f"- beyond the track table: **{len(beyond)}** "
           f"in {len({r['file'] for r in beyond})} file(s)",
           f"- lost with pointers that resolve: **{len(lost)}** "
           f"in {len({r['file'] for r in lost})} file(s)", ""]

    out += ["## By cause", "",
            "The queue. `beyond_table` is listed for completeness and is not "
            "part of it.", "",
            "| subtunes | files | fate | cause |", "|---:|---:|---|---|"]
    groups = defaultdict(list)
    for r in rows:
        if r["fate"] in ("emitted",):
            continue
        groups[(r["fate"], r["why"])].append(r)
    for (fate, why), rs in sorted(groups.items(),
                                  key=lambda kv: -len(kv[1])):
        out.append(f"| {len(rs)} | {len({r['file'] for r in rs})} | "
                   f"`{fate}` | {why} |")
    out.append("")

    out += ["## Lost subtunes where at least one pointer resolves", "",
            "Read the pointers, not the counts. `!` marks one that does not "
            "resolve; the last emitted subtune's row is shown first for "
            "comparison, and a real table row sits beside its neighbours.",
            ""]
    ...
    def _ptrs(r):
        return " ".join(
            ...
            for p, ok in zip(r.get("pointers", [None] * 3),
                             r.get("resolved", [False] * 3)))

    for f, rs in sorted(interesting.items(), key=lambda kv: -len(kv[1])):
        decl = rs[0]["declared"]
        kept = [r for r in rows if r["file"] == f and r["fate"] == "emitted"]
        out += [f"### {f} — {decl} declared, {len(kept)} emitted, "
                f"{len(rs)} lost", "",
                "| subtune | fate | pointers | refs | dangling |",
                "|---:|---|---|---:|---:|"]
        if kept:
            last = max(kept, key=lambda r: r["subtune"])
            out.append(f"| {last['subtune']} | *last emitted* | "
                       f"{_ptrs(last)} | {last['refs']} | {last['dangling']} |")
        for r in sorted(rs, key=lambda r: r["subtune"]):
            if not r["voices_ok"]:
                continue
            out.append(f"| {r['subtune']} | {r['fate']} | {_ptrs(r)} | "
                       f"{r['refs']} | {r['dangling']} |")
        out.append("")

    dead = Counter(r["file"] for r in lost
                   if not r["voices_ok"])
    if dead:
        out += ["## Files whose lost subtunes have no resolving pointer at all",
                "",
                "Nothing to convert: every pointer lands outside the file. "
                "These are header padding, not music.", "",
                "| file | lost |", "|---|---:|"]
        for f, n in sorted(dead.items(), key=lambda kv: -kv[1]):
            out.append(f"| {f} | {n} |")
        out.append("")
    return "\n".join(out)
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--subtune-census", metavar="PATH",
                        help="instead of the survey, write a census of every "
                             "declared subtune and what became of it. Reads "
                             "the record convert_tracks fills in as it decides"
                             ", so it cannot disagree with the converter")
    ...
    if args.subtune_census:
        rows = subtune_census(paths)
        Path(args.subtune_census).write_text(
            build_subtune_census(rows, sid_dir, __version__), encoding="utf-8")
        lost = sum(1 for r in rows
                   if r["fate"] in ("trimmed", "placeholder"))
        live = sum(1 for r in rows
                   if r["fate"] in ("trimmed", "placeholder") and r["voices_ok"])
        print(f"{len(rows)} declared subtune(s), {lost} lost "
              f"({live} with a pointer that resolves) -> {args.subtune_census}")
        return 0
