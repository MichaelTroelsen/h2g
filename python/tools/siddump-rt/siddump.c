...
int main(int argc, char **argv)
{
  ...
  while (frames < firstframe + seconds*50)
  {
    ...
    // Run the playroutine once per call the tune makes in a frame. A row of
    // this dump is one PAL frame of *real time* whatever the call rate, so a
    // multispeed tune stays on the same time axis as a single-speed one and
    // the two can be compared frame for frame.
    for (call = 0; call < callsperframe; call++)
    {
      ...
      // initcpu zeroes cpucycles, so the profiling column has to accumulate
      // here or it would report only the frame's last call.
