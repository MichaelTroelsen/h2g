# LESSONS.md — the full CLAUDE.md, archived

This is the verbatim text of `CLAUDE.md` as it stood at **v0.5.475 (commit
d3775b4)**, before it was compacted into an index. Nothing here has been
edited: every measured figure, every retraction, every grading pass is as it
was written, with whatever staleness it carried.

It is **not** loaded into an agent's context automatically. `CLAUDE.md` now
carries the rules; this file carries the evidence behind them. When a rule in
`CLAUDE.md` needs its numbers, its worked example, or the session that produced
it, grep here — the wording of every rule was preserved on both sides so a grep
for a phrase in one lands in the other.

New lessons go in `CLAUDE.md` as a rule, and their evidence here.
