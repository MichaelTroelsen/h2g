...
`presets.json` carried **47 `--fidelity` settings forward**. The last full
search was **v0.5.271** (83 songs, 31 combinations, 60 s: 2 gained, 0 lost),
plus a one-song search for Powerplay at v0.5.278. The converter has since
changed what it emits on three paths — the hard restart (v0.5.275, five files
gaining 25–45 points of melody), the outer-gate-only row (v0.5.289) and the
vibrato delay (v0.5.294, 25 files). By this project's own standard — a setting
chosen against an emitter that no longer exists is not a measurement — those
47 are due a re-run. Four hours, and `[main]` work.
