...
* **v0.5.266** re-ran the four-hour `--fidelity` search against the changed
...
A per-song toggle would settle it, at the cost of doubling a four-hour search.
...
* A per-song `HARD_RESTART` row bound (+1.6pp gate, doubles a four-hour search).
...
* Repo `C:\Users\mit\claude\h2g`, branch `master`, HEAD `60ec0c4`, in sync with
  `origin/master`.
...
* **HEAD is `60ec0c4` v0.5.297, pushed; master in sync with `origin/master`.**
* Working tree clean but for untracked `6581.pdf`.
* `Commando.sng` byte-exact.
* Last suite at HEAD: **1135 passed, 3 skipped**.

## Generated artefacts

| file | stamp | current? |
|---|---|---|
| `SURVEY.md` | 0.5.294 | yes |
| `presets.json` | 0.5.294 | yes |
| `FIDELITY.md` | 0.5.294 | yes |
| `SUBTUNES.md` | 0.5.294 | yes (on demand) |
| `VIBRATO.md` | 0.5.297 | yes (on demand) |

v0.5.295–297 changed **no converter code** — they are census corrections — so
the four stamped 0.5.294 are accurate in content and merely lag in stamp. Note
that `test_preset_passthrough`'s guard is armed only while presets.json's stamp
equals `__version__`, so it is **skipping right now**; a regeneration after the
next bump re-arms it. That is one of the three skips.
